...
class EmpInfo extends CActiveRecord
{
	...
	public function rules()
	{
		...
		return array(
			array('empOrNot, occupation, workplaceName, currJobRelToCourse, accountID', 'required'),
			array('empOrNot, currJobRelToCourse, unemploymentReason, accountID', 'numerical', 'integerOnly'=>true),
			array('occupation, workplaceName, workplaceAdd', 'length', 'max'=>45),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('empInfoID, empOrNot, occupation, workplaceName, workplaceAdd, currJobRelToCourse, unemploymentReason, accountID', 'safe', 'on'=>'search'),
		);
	}
